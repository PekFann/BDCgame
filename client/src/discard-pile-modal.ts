...
let lastKnownTopCardId: string | null = null;
...
function ensureDiscardModal(): { root: HTMLElement; panel: HTMLElement } {
  ...
  modalEl.innerHTML = `
    ...
    <div class="discard-pile-panel modal-panel">
      <h3 class="card-modal-title">Discard Pile</h3>
...
function resolveTopCardId(pub: PublicGameState, count: number): string | null {
  const discard = getActionDiscard(pub);
  const top = discard[discard.length - 1];
  if (top) {
    lastKnownTopCardId = top.cardId;
    return top.cardId;
  }
  if (count > 0 && lastKnownTopCardId) return lastKnownTopCardId;
  return null;
}
...
export function renderDiscardPileSlot(
  el: HTMLElement,
  pub: PublicGameState,
  ...
  const topCardId = resolveTopCardId(pub, count);
  const renderKey = `${count}-${topCardId ?? ""}`;
  if (renderKey === lastRenderedKey && el.firstElementChild) return;
  ...
  btn.title =
    count === 0
      ? "Browse Discard Pile (empty)"
      : `Browse Discard Pile (${count})`;

  if (hasCards) {
    const topImg = topCardId ? cardImg(topCardId) : "";
    const topCardMarkup = topCardId
      ? `<img class="discard-top-card" src="${topImg}" alt="" />`
      : `<span class="discard-top-card discard-top-card--placeholder" aria-hidden="true"></span>`;
    btn.innerHTML = `
    ...
    <span class="hand-discard-label">Discard</span>
  `;
  } else {
    btn.innerHTML = `
  ...
  btn.addEventListener("click", () => {
    ...
    if (latest) openDiscardPileModal(latest);
  });
  el.appendChild(btn);
}
